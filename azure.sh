...
GH_REPO="JChorao/CryptoTracker" # Confirme se este é o seu repositório

# Garante que o Git Bash no Windows não estraga caminhos que comecem por "/"
export MSYS_NO_PATHCONV=1

echo "------------------------------------------------------------------"
echo "🚀 SETUP FINAL: INFRAESTRUTURA + NODE 24 + AUTO-DEPLOY"
echo "------------------------------------------------------------------"

echo "📌 A criar Grupo de Recursos..."
az group create --name "$AZ_RG" --location "$AZ_LOCATION"

echo "📌 A criar Cosmos DB..."
...
echo "📌 A criar App Service (Node 24)..."
...
echo "📌 A gerar Service Principal para o GitHub..."
...
echo "------------------------------------------------------------------"
# Pequena pausa para garantir que os segredos estão propagados no GitHub
sleep 5 

# Tenta relançar as runs mais recentes dos workflows de deploy
