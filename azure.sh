...
az cosmosdb sql container create --account-name "$AZ_COSMOS_ACCOUNT" --resource-group "$AZ_RG" \
    --database-name "$AZ_COSMOS_DB" --name "$AZ_COSMOS_CONTAINER" --partition-key-path "/partitionKey" --throughput 400
...
echo "📌 A criar App Service (Node 20 LTS)..."
az appservice plan create --name "plan-crypto" --resource-group "$AZ_RG" --sku B1 --is-linux
az webapp create --name "$AZ_APP_NAME" --resource-group "$AZ_RG" --plan "plan-crypto" --runtime "NODE|20-lts"
...
echo "✅ Setup concluído. URL: $APP_URL"